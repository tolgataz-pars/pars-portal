...
class AppState {
        ...
        if (students.length === 0) {
            container.innerHTML = `
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center text-slate-500 space-y-3">
                    ...
                    <p class="font-medium text-white text-base">Bu Sınıfta Öğrenci Kaydı Bulunamadı</p>
                    <p class="text-xs text-slate-400">Not ve beceri analizi eklemek için önce sınıfa öğrenci kaydı oluşturunuz.</p>
                </div>
            `;
            if (window.lucide) window.lucide.createIcons();
            return;
    ...
    openSkillGradeModal(studentId, skillKey = 'Reading') {
        ...
        if (document.getElementById('modalExamFinal')) document.getElementById('modalExamFinal').value = fScore;
        if (document.getElementById('modalFinalScore')) document.getElementById('modalFinalScore').value = fScore;

        if (document.getElementById('modalExamHwAnalysis')) document.getElementById('modalExamHwAnalysis').value = hwAnalysis;
        if (document.getElementById('modalHomeworkAnalysis')) document.getElementById('modalHomeworkAnalysis').value = hwAnalysis;

        if (document.getElementById('modalGradeFileUrl')) document.getElementById('modalGradeFileUrl').value = fileUrl;
    ...
    downloadStudentExamFile(studentId, skill) {
        ...
        const fileData = item.fileUrl || (this.data.students && this.data.students.find(s=>s.id===studentId)?.fileData);
        const fileName = item.fileName || (this.data.students && this.data.students.find(s=>s.id===studentId)?.fileName) || `${skill}_Rapor`;
        ...
        this.showToast(`📁 ${fileName} indirmesi başlatıldı.`, "success");
    }
    ...
    async exportExcel() {
        ...
        if (window.XLSX) {
            ...
            const sheetData = [
                ...
                ["Sınıf:", currentClass.name, "", "Yoklama Yapan Öğretmen:", this.data.currentUser ? this.data.currentUser.name : (currentClass.teacher || "Dönüşümlü Öğretmen")],
